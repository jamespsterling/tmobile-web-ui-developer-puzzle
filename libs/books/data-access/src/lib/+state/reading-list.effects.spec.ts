...
describe('ToReadEffects', () => {
  ...
  describe('removeBook$', () => {
    it('should work', done => {
      ...
      httpMock.expectOne({url: `/api/reading-list/${item.bookId}`, method: 'DELETE'}).flush([]);
    });
  });
  ...
  describe('finishBook$', () => {
    it('should work', done => {
      ...
      actions = new ReplaySubject();
      actions.next(ReadingListActions.markFinished({ item }));

      effects.finishBook$.subscribe(action => {
        expect(action).toEqual(
          ReadingListActions.confirmMarkFinished({ item })
        );
        done();
      });

      httpMock.expectOne({url: `/api/reading-list/${item.bookId}/finished`, method: 'PUT'}).flush([]);
    });
  });
});
